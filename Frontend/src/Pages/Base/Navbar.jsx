  ...
  return (
    <div className='__Navbar__ container'>
        ...
        <ul className="Links">
            ...
            { account == null ? ( <Link to='/login' className="link_item">Login</Link> ) : ( <button className="link_item" onClick={()=>logout()}>Logout</button> ) }
            { account ? ( <Link to={`/profile/${account.id}/`}>{ account.username }</Link> ) : '' }
        </ul>
    </div>
  )
}
